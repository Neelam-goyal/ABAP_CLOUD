...
CLASS ZCL_GOODS_MOVEMENT_CREATE IMPLEMENTATION.
  ...
  METHOD post_goods_mvt.
    ...
    IF im_item[] IS NOT INITIAL.
      ...
      lv_post_date = sys_date+0(4) && '-' && sys_date+4(2) && '-' && sys_date+6(2) && 'T00:00:00'.
      ...
      LOOP AT im_item INTO DATA(ws_gmvt).
        ...
        lv_str_itm = lv_str_itm
                ...
                && '"Material":' && '"' && ws_gmvt-material && '",'   "IRS0022200XS12",'
                && '"Plant":' && '"' && ws_gmvt-plant && '",'         "1001",'
                && '"StorageLocation":' && '"' && ws_gmvt-storagelocation && '",'   "S022",'
                && '"GoodsMovementType": "101",'
                && '"PurchaseOrder":' && '"' && ws_gmvt-purchaseorder && '",'           "4500000215",'
                && '"PurchaseOrderItem":' && '"' && ws_gmvt-purchaseorderitem && '",'   "00010",'
                && '"GoodsMovementRefDocType": "B",'
*                && '"IsCompletelyDelivered": false,'
                && '"QuantityInEntryUnit":' && '"' && ws_gmvt-quantityinentryunit && '"' "10",'
*                && '"IssgOrRcvgBatch":"supplbatch",'
*                && '"ShelfLifeExpirationDate":' && lv_doc_date "/Date(1492041600000)/",
*                && '"ManufactureDate":' && lv_doc_date ""/Date(1492041600000)/",
                && lv_str_othr1.
