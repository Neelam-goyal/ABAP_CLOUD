...
CLASS ZCL_MM_CUSTOM_PRINT IMPLEMENTATION.
     ...
     IF sy-subrc EQ 0.
       ...
       IF iv_action = 'misprint'.
         ...
         LOOP AT lt_hdr INTO DATA(ls_hdr).
           ...
           gs_grn-plant_addl1          = '12/2, MAIN MATHURA ROAD,FARIDABAD,HARYANA-121003'  ##NO_TEXT.
           gs_grn-plant_addl2          = 'Website: www.sterlinggtake.com'  ##NO_TEXT.
           gs_grn-plant_addl3          = ''.
       ...
       ELSE.
         ...
         LOOP AT lt_hdr INTO ls_hdr.
           ...
           READ TABLE lt_ge_data INTO DATA(ls_ge_det) INDEX 1. "#EC CI_NOORDER
           gs_grn-plant                = ls_hdr-Plant.
           gs_grn-materialdocument     = ls_hdr-materialdocument.
           gs_grn-materialdocumentyear = ls_hdr-materialdocumentyear.
           gs_grn-documentdate         = ls_hdr-documentdate.
           ...
           gs_grn-ge_num               = ls_ge_det-gentry_num.

           gs_grn-ge_date              = ls_ge_det-erdat+6(2) && '.' &&
                                         ls_ge_det-erdat+4(2) && '.' &&
                                         ls_ge_det-erdat+0(4).
           ...
           LOOP AT lt_grn INTO ls_grn WHERE materialdocument = ls_hdr-materialdocument
             ...
             IF ls_grn-entryunit = 'ST'.
               gs_item-unit                  = 'PC'.
             ELSE.
               gs_item-unit                  = ls_grn-entryunit.
             ENDIF.

             gs_item-po_num                = ls_grn-purchaseorder.
   ...
   METHOD get_resb_data.
     ...
     IF iv_rsnum IS NOT INITIAL.
       ...
       IF ls_resb_hdr IS NOT INITIAL.
         ...
         gs_resb-plant  = cs_mseg-Plant.
         SELECT SINGLE  materialdocument , postingdate , createdbyuser, materialdocumentheadertext  FROM i_materialdocumentheader_2
       ...
       READ TABLE lt_resb_item INTO DATA(cs_resb_item) INDEX 1. "#EC CI_NOORDER
       gs_resb-plant           = cs_resb_item-plant.
       ...
       gs_resb-comp_name       = 'BHAGWATI LACTO VEGETARIAN EXPORTS PVT. LTD.' ##NO_TEXT.
       gs_resb-slip_name       = 'Material Issue Slip' ##NO_TEXT .
       gs_resb-uom             = ''.
       gs_resb-mdoc_hdrtxt     = ls_mkpf-materialdocumentheadertext.

       READ TABLE lt_mseg INTO DATA(ls_mseg_1)
       WITH KEY reservation = ls_resb_hdr-reservation
                debitcreditcode = 'H'.

       "gs_resb-req_loc       = ls_resb_item-StorageLocation.
       gs_resb-issue_from    = ls_mseg_1-storagelocation.
       ...
       LOOP AT lt_resb_item INTO DATA(ls_resb_item).
         ...
         ls_item-prododr_num   = ''.
         IF ls_resb_item-baseunit = 'ST'.
           ls_item-item_uom      = 'PC'.
         ELSE.
           ls_item-item_uom      = ls_resb_item-baseunit.
         ENDIF.
   ...
   METHOD prep_xml_grn_print.
     ...
     IF ls_final-plant = '1101' ##NO_TEXT.
       sub_heading = '12/2, Mathura Road,Faridabad - 121003' ##NO_TEXT.
     ELSEIF ls_final-plant = '1102' ##NO_TEXT.
       sub_heading = 'NO. 73 74 113 114, ND Fusion Mall, 1ST PHASE, 16th Main Road, BTM Layout Stage 2, Bengaluru, Bengaluru Urban, Karnataka - 560076' ##NO_TEXT.
     ENDIF.
     ...
     DATA(lv_xml) =  |<Form>| &&
                     ...
                     |<Plant_code>{ ls_final-plant }</Plant_code>| &&
   ...
   METHOD prep_xml_issue_slip_print.
     ...
     DATA : heading       TYPE c LENGTH 100,
            sub_heading   TYPE c LENGTH 200,
            lv_vech_no    TYPE c LENGTH 60,
            lv_place      TYPE c LENGTH 60,
            lv_recd_bags  TYPE c LENGTH 60,
            lv_remark     TYPE c LENGTH 60,
            lv_no_bags    TYPE c LENGTH 60,
            lv_quality    TYPE c LENGTH 60,
            lv_short_bags TYPE c LENGTH 60,
            lv_pkg_detl   TYPE c LENGTH 60,
            lv_palleder   TYPE c LENGTH 60,
     ...
     IF ls_final-plant = '1001'.
       ...
       lv_plant_adrs = 'Works: Rural Focal Point Mana Singh wala , Firozepur Moga Road 152001' ##NO_TEXT.
     ELSEIF ls_final-plant = '2001'.
       heading     = 'BHAGWATI LACTO FOODS PVT LTD.' ##NO_TEXT.
       lv_plant_adrs = 'Works : VILL. RUKNA MUNGLA, FARIDKOT ROAD, FEROZEPUR, Ferozepur, Punjab, 152001' ##NO_TEXT.
     ELSEIF ls_final-plant = '3001'.
       heading     = 'HEALTHY HARVESTED FOODS PRIVATE LIMITED' ##NO_TEXT.
       lv_plant_adrs = 'Works : PLOT NO100, SECTOR 1/A, GANDHIDHAM, Kachchh, Gujarat, 370201' ##NO_TEXT.
     ENDIF.

     lv_vech_no      = ls_final-mdoc_hdrtxt.
     lv_place        = ''.
     lv_recd_bags    = ''.
     lv_remark       = ''.
     lv_no_bags      = ''.
     ...
     DATA(lv_xml) =  |<Form>| &&
                     ...
                     |<sum_pend_qty>{ ls_final-sum_pend_qty }</sum_pend_qty>| &&
                        |<vech_no>{ lv_vech_no }</vech_no>| &&
                        |<place>{ lv_place }</place>| &&
                        |<recd_bags>{ lv_recd_bags }</recd_bags>| &&
                        |<remark>{ lv_remark }</remark>| &&
                        |<no_bags>{ lv_no_bags }</no_bags>| &&
                        |<quality>{ lv_quality }</quality>| &&
                        |<short_bags>{ lv_short_bags }</short_bags>| &&
                        |<pkg_detl>{ lv_pkg_detl }</pkg_detl>| &&
                        |<palleder>{ lv_palleder }</palleder>| &&
                     |<ItemData>| ##NO_TEXT.
