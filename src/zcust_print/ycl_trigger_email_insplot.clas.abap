...
inspectionlotusagedecidedon
       ...
       FROM zi_insp_lot_rej
               WHERE inspectionlot IN @rt_lot INTO TABLE @DATA(gt_lot). "#EC CI_NO_TRANSFORM
    ELSE.
...
inspectionlotusagedecidedby,
inspectionlotusagedecidedon
      FROM zi_insp_lot_rej WHERE inspectionlotusagedecidedon = @lv_date INTO TABLE @gt_lot. "#EC CI_NO_TRANSFORM
      ...
      LOOP AT gt_lot INTO DATA(gs_lot). "WHERE InspLotQtyToBlocked IS NOT INITIAL.
        ...
        IF lt_email[] IS NOT INITIAL.
          ...
          TRY.
              ...
              lo_mail->set_subject( |SUPPLIER RECEIPT INSPECTION CONFIRMATION - | && lv_docnum ) ##NO_TEXT.

              lv_post_date = gs_matdoc-postingdate+6(2) && '.' && gs_matdoc-postingdate+4(2) && '.'
                             && gs_matdoc-postingdate+0(4).

              DATA(lv_mail_body) = '<p>Dear Sir,</p>'
               && '<p>Your material has been received at SIPL and status given below</p>'.
...
*              && '<p>Requested you to please proceed for required action</p>'.
              ...
              && '<tr>'
              && '<td>' && gs_lot-purchasingdocument && '</td>'
              && '<td>' && gs_matdoc-productdescription && '</td>'
              && '<td>' && gs_matdoc-suppliername && '</td>'
              && '<td>' && lv_refdoc && '</td>'
              && '<td>' && gs_lot-inspectionlotquantity && '</td>'
              && '<td>' && gs_lot-insplotqtytoblocked && '</td>'
              && '</tr>'.                               "#EC CI_NOORDER

              lv_table1_data  = lv_table1_data  && '</tbody></table>'. "#EC CI_NOORDER
              lv_final_table1 = lv_table1_header && lv_table1_data && '<br>'. "#EC CI_NOORDER
...
*             DATA(lv_final_mail_body)  = lv_mail_body && lv_body_data && lv_footer.
              DATA(lv_final_mail_body)  = lv_mail_body && lv_final_table1 && lv_footer.
