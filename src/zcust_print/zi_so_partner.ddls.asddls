...

{
  ...
  key so_partner.SalesDocument,
      ...
      max( case so_partner.PartnerFunction when 'AG' then tax_i_so.BPTaxNumber end )                                 as AG_TAX,
      max( case so_partner.PartnerFunction when 'AG' then so_partner.Customer end )                                  as AG_code,

      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.AddresseeFullName end )                          as SP_NAME,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.CityName end )                                   as SP_CITY,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.PostalCode end )                                 as SP_PIN,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.Street end )                                     as SP_STREET,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.StreetName end )                                 as SP_STREET1,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.HouseNumber end )                                as SP_HOUSE_NO,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.Country end )                                    as SP_COUNTRY,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.Region end )                                     as SP_REGION,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.FormOfAddress end )                              as SP_FROM_OF_ADD,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so._EmailAddress.EmailAddress end )                 as SP_EMAIL,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so._PhoneNumber.PhoneAreaCodeSubscriberNumber end ) as SP_PHONE4,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.StreetPrefixName1 end )                          as SP_StreetPrefixName1,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.StreetPrefixName2 end )                          as SP_StreetPrefixName2,
      max( case so_partner.PartnerFunction when 'SP' then adrs_i_so.StreetSuffixName1 end )                          as SP_StreetSuffixName1,
      max( case so_partner.PartnerFunction when 'SP' then tax_i_so.BPTaxNumber end )                                 as SP_TAX,
      max( case so_partner.PartnerFunction when 'SP' then so_partner.Customer end )                                  as SP_code
}
group by
